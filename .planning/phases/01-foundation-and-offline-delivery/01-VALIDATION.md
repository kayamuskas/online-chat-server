...
| 01-01-02 | 01 / Task 2 | 1 | OPS-02, ARCH-01, ARCH-02 | T-01-02, T-01-13 | Shared runtime and queue contracts stay centralized, and the Docker dependency path includes a documented lockfile-linked refresh/verify procedure. | static | `rg -n "QUEUE_NAMES|SYSTEM_JOB_NAMES|SERVICE_PORTS|pnpm-lock.yaml|frozen-lockfile|verify|docker compose" packages/shared/src/index.ts packages/shared/src/config.ts packages/shared/src/queue.ts docs/offline-runtime.md` | `packages/shared/src/*`, `docs/offline-runtime.md` | ✅ green |
...
| 01-04-01 | 04 / Task 1 | 3 | OPS-01, OPS-02 | T-01-10, T-01-11, T-01-12, T-01-14 | Compose uses health-gated startup, read-only defaults, and Dockerfiles constrained to the documented lockfile-backed dependency path. | smoke/static | `rg -n "^services:|service_healthy|read_only: true|pnpm-lock.yaml|--frozen-lockfile" infra/compose/compose.yaml infra/docker/api.Dockerfile infra/docker/web.Dockerfile infra/docker/worker.Dockerfile` | `infra/compose/compose.yaml`, `infra/docker/*.Dockerfile` | ✅ green |
...
- `scripts/qa/phase1-offline-check.sh` - covers OPS-02 plus lockfile-backed dependency verification.
...
|----------|-------------|------------|-------------------|
| Fresh-clone startup with Docker build-time dependency downloads | OPS-02 | Depends on local Docker daemon, image registry access, and host network conditions. | Run `docker compose up --build --wait` from a clean checkout and confirm the stack comes up without any extra setup beyond Docker/network access. |
